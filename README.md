...
RF Academy Pro 是一套手機優先、可安裝、可離線複習的 180 天工程英文 PWA。課程涵蓋 RF、EMC、Safety、FCC、CE、NCC、BSMI、UL、IEC、EN、ISO/IEC 17025、測試報告、實驗室溝通、客戶會議與工程師英文面試。

## 完整功能

- Day001–Day180，每天包含 10 個專業詞彙、5 句常用句、1 段情境對話、1 個 Debug Case、1 題 Interview Question 與 5 題混合測驗。
...
- Dashboard、180 天進度地圖、連續學習日、測驗平均與複習清單。
...
啟動器會先確認 Day001–Day180；資料缺漏時會自動重新產生，再啟動 `http://127.0.0.1:8080/` 並開啟瀏覽器。
...
5. 每次推送會先確認或重建 Day001–Day180、執行全量驗證，再發布網站。
...
產生器會重建 `data/lessons/day001.json` 至 `day180.json`、課程索引、8 份分類詞庫、面試題庫與 Debug 題庫。產生過程不需要網路或 API Key，結果可重現。
...
js/interview.js
js/debug.js
data/lessons/day001.json ... day180.json
